...
export async function uploadImg(req: Request, res: Response, title: string) {
    ...
    if (allowedExtensions.includes(fileExt.toLowerCase())) {
        ...
        return 'Fayl formati qo\'llanilganlardan farq qiladi'
    }
    const imgDir = path.join(process.cwd(), 'src', 'public', 'imgs');
    if (fs.existsSync(imgDir)) {
        try {
            await uploadedFile.mv(path.join(imgDir, dynamicFileName));
            const img_link = `/imgs/${dynamicFileName}`; // Fayl manzili
            return img_link;
        } catch (error) {
            return 'Faylni saqlashda xatolik yuz berdi'
        }
    }
